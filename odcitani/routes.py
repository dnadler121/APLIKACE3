...
@odcitani_bp.route("/rozklad", methods=["GET","POST"])
def rozklad():
    ...
    if request.method == "POST":
        ...
        if request.form.get("finish"):
            ...
            if hotovo:
                return redirect(url_for("odcitani.skrtani"))
            ...
            try:
                ...
                if target is None:
                    ...
                    return redirect(url_for("odcitani.rozklad"))

    return render_template(
        "odcitani_rozklad.html",
        rows=rows,
        divisors=DIVISORS,
...
@odcitani_bp.route("/skrtani",methods=["GET","POST"])
def skrtani():
...
@odcitani_bp.route("/citatele", methods=["GET","POST"])
def citatele():
    ...
    if request.method == "POST":
        ...
        if value == f"{c1}-{c2}":
            return redirect(url_for("odcitani.vysledek"))

        else:
            error = "Špatný zápis."

    return render_template(
        "odcitani_citatele.html",
        a=session["a"],
        b=session["b"],
        d1=session["d1"],
        d2=session["d2"],
...
@odcitani_bp.route("/vysledek", methods=["GET","POST"])
def vysledek():
    ...
    if request.method == "POST":
        ...
        if value == str(result):
            ...
            if session["completed"] >= 10:
                return redirect(url_for("odcitani.konec"))

            return redirect(url_for("odcitani.dalsi"))

        else:
            error = "Špatně."

    return render_template(
        "odcitani_vysledek.html",
        c1=session["c1"],
        c2=session["c2"],
        nsn=session["nsn"],
        error=error
